...
'0.32 ->  Bug fixes: File writer wont append name correctly | Simplify JSON structure | Mapping file for lisences | move mapping files into mapping folder #25/04/2023',
...
def create_asset_json(*args): # args: ['file' / 'dir'], [path], [mapping], [csv files]
    ...
    mapping_type = args[2]
    if mapping_type == 'sw':
        sw_mapping = get_mapping('sw') #TODO: Bug 1 - These arguments should be taken into account
    ...
    elif mapping_type == 'license':
        lic_mapping = get_mapping('license')
        lic_mapping = lic_mapping[3]
        lic_mapping_values = lic_mapping.values()
...
# START ------ commented out in version 0.321 ------
        for x in lic_mapping_values:
            ...
            if value_in_list == 0:
                    ...
                    #lic_full_dict = {lic_main_value : {lic_mapping_dict}} #added in version 0.321
                    #lic_main_value_dict[lic_main_value].append(lic_mapping_dict)

        #asset_json_dict[file_name_short].append(lic_main_value_dict)
...
# START ------ commented out in version 0.321 ------
                    ...
                    if upper_element != None:
                        ...
                        #asset_json_dict[file_name_short][index_num][upper_element][sub_indx][component] = unit_dict
...
def arg_parser():
    ...
    parser.add_argument('-m','--mapping',help='Defines what mapping should be used [sw/hw]', nargs='+') #depricated in version 0.32
...
if __name__ == '__main__':
    ...
    if to_process[2] is None:
        mapping = mapping
        check_file_or_folder_exists('mapping/sw_mapping.ini', 'file')
        check_file_or_folder_exists('mapping/hw_mapping.ini', 'file')
        check_file_or_folder_exists('mapping/license_mapping.ini', 'file')
